...
**Архивное обновление (legacy):** ранее были добавлены `24` теста для `HistoryManager` (`tests/core/HistoryManager.baseline.test.js`) — добавление команд в историю, undo/redo, merge, maxHistorySize, служебные методы.
С переходом на append-only историю через API v2 этот файл удален и больше не используется в актуальном наборе тестов.
...
### `tests/image-object2/NoteTool.e2e.spec.js` — 19 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/image-object2/ImageTool.e2e.spec.js` — 12 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/image-object2/EmojiTool.e2e.spec.js` — 13 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/image-object2/FileTool.e2e.spec.js` — 15 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/image-object2/ShapesTool.e2e.spec.js` — 16 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/image-object2/FrameTool.e2e.spec.js` — 16 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/image-object2/GroupSelection.e2e.spec.js` — 10 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### Дополнительные удаленные legacy E2E (архив)

Эти файлы также удалены и не используются в текущем прогоне, но здесь зафиксированы для исторического контекста:

- `tests/image-object2/MindmapTool.e2e.spec.js`
- `tests/image-object2/GroupSelection.undo-redo-stress.e2e.spec.js`
...
### `tests/image-object2/DrawingTool.e2e.spec.js` — 12 E2E-тестов *(LEGACY, не используется; файл удален)*
...
### `tests/core/HistoryManager.baseline.test.js` — 24 теста *(LEGACY, не используется; файл удален)*
